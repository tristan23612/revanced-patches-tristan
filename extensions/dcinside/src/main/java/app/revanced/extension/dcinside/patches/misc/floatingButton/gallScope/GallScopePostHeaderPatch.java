...
public class GallScopePostHeaderPatch {
    ...
    public static void setGallScopeClickListener(View postHeaderView) {
        if (postHeaderView == null || !Settings.SHOW_USER_ID.get() || !Settings.SHOW_GALL_SCOPE_BUTTON.get()) return;

        // 1. 동적 리소스 ID 탐색 및 예외 처리
        int resId = postHeaderView.getResources().getIdentifier(
                "custom_user_id",
                "id",
                postHeaderView.getContext().getPackageName()
        );
        if (resId == 0) return;

        TextView userIdView = postHeaderView.findViewById(resId);
